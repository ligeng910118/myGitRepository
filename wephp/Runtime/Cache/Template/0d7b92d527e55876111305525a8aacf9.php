      ...
      <div id="main">
		...
		$(document).ready(function(){
			$('#getValue').change(function(){
				//alert('ok');
				var template_id = $('#getValue option:selected').val();
              	var url = window.location.href;
				var sendUrl = url.replace(/singleMessage/,"singleMessageTemplate");
				//alert(sendUrl);
				...
				$.ajax({
				    ...
				    success: function(data){
				    	...
				    	var jsonLength = getHsonLength(data);
				    	//alert(data[1][1]);
				        var nr = '';
				        for(var i=1;i<=jsonLength;i++)
				      		nr += data[i][0]+'：<br><input type="text" class="text input-large" name='+data[i][1]+' value="" ><br />';
				      		$('#templateNum').html(nr);
				    },
				 })
			})
		...
		function getHsonLength(json){
	 	...
	 	<div class="mainRight">
	 		<div class="rightTitle">单发消息</div>
	 		<div>
	 		<?php if($_GET['id']== null): ?><div>单发模板：
	 				<select id="getValue">
	 					<?php if(is_array($list)): foreach($list as $k=>$vo): ?><option value="<?php echo ($vo["template_id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; ?>
	 				</select>
	 			</div><?php endif; ?>
	 			<div>
	 			<form method="post" action="index.php?s=/w16/Template/Template/singleMessageAction">
	 			<input type="hidden" id="template_id" name="template_id" value="<?php echo ($list[0]["template_id"]); ?>" />
	 				<input type="hidden" name="project" value="<?php echo ($address); ?>" />
	 				首标题: <br><input type="text" class="text input-large" name="first" value="<?php echo ($data["first"]); ?>" /><br>
	 				<div id="templateNum">
	 				
					股票名称 ：<br><input type="text" class="text input-large" name="keyword1" value="<?php echo ($data["keyword1"]); ?>" ><br />
					股票代码 ：<br><input type="text" class="text input-large" name="keyword2" value="<?php echo ($data["keyword2"]); ?>" ><br />
					操作类型 ：<br><input type="text" class="text input-large" name="keyword3" value="<?php echo ($data["keyword3"]); ?>" ><br />
					买入价格 ：<br><input type="text" class="text input-large" name="keyword4" value="<?php echo ($data["keyword4"]); ?>" ><br />
					交易时间 ：<br><input type="text" class="text input-large" name="keyword5" value="<?php echo ($data["keyword5"]); ?>" ><br />
	 				</div>
	 				尾备注：<br><input type="text" class="text input-large" name="remark" value="<?php echo ($data["remark"]); ?>"><br />
	 				详情url(必须为http://开头)：<br><input type="text" class="text input-large" name="url" value="<?php echo ($data["url"]); ?>"><br />
