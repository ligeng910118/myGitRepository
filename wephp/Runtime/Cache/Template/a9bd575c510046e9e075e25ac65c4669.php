...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
      ...
      <div id="main">
	 	...
	 	<div class="mainRight">
	 		<div class="rightTitle">群发消息</div>
	 		<form method="post" action="">
	 		<?php if(is_array($list)): foreach($list as $key=>$vo): ?><div class="listRight messageManage">
	 			<div>标题：<?php echo ($vo["first"]); ?></div>
	 			<div>内容一：<?php echo ($vo["keyword1"]); ?></div>
	 			<div>内容二：<?php echo ($vo["keyword2"]); ?></div>
	 			<?php if($vo["keyword3"] != null): ?><div>内容三：<?php echo ($vo["keyword3"]); ?></div><?php endif; ?>
	 			<?php if($vo["keyword4"] != null): ?><div>内容四：<?php echo ($vo["keyword4"]); ?></div><?php endif; ?>
	 			<?php if($vo["keyword5"] != null): ?><div>内容五：<?php echo (msubstr($vo["keyword5"],0,40)); ?></div><?php endif; ?>
	 			<div>备注：<?php echo (msubstr($vo["remark"],0,20)); ?></div>
	 			<div>请选择模板：<input type="radio" name="id" value="<?php echo ($vo["id"]); ?>" /></div>
	 		</div><?php endforeach; endif; ?>
	 			 	<div style="clear:both"><input type="submit" class="btn" value="群发"></div>
	 		</form>
	 	</div>
